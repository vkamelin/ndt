...
@section('content')
    <div class="grid gap-4 lg:grid-cols-[1.25fr_0.75fr]">
        <section class="rounded-3xl border border-white/10 bg-white/5 p-5 shadow-2xl shadow-slate-950/25">
            <h2 class="text-lg font-semibold text-white">Текущая смена</h2>
            <p class="mt-1 text-sm text-slate-300">Операции с пленкой, химией и регламентными работами.</p>

            @if ($currentShift !== null)
                <div class="mt-4 rounded-2xl border border-sky-400/20 bg-sky-500/10 p-4">
                    <p class="text-sm text-sky-100">Смена №{{ $currentShift->id }} · {{ $currentShift->status->label() }}</p>
                ...
                <div class="mt-4 rounded-2xl border border-dashed border-white/15 bg-slate-900/40 p-4">
                    ...
                    @if ($employee !== null)
                        <form method="post" action="{{ route('admin.shifts.store') }}" class="mt-4 flex flex-wrap gap-2">
                            ...
                            <input type="hidden" name="comment" value="Пуск смены из PWA">
                            <button class="rounded-full bg-sky-500 px-4 py-2 text-sm font-medium text-white">Открыть смену</button>
                        </form>
                    @endif
                </div>
            @endif
        ...
        <aside class="space-y-4">
            ...
            <div class="rounded-3xl border border-white/10 bg-white/5 p-5">
                ...
                <p class="mt-2 text-sm text-slate-300">Детальную фиксацию операций удобнее продолжать в полной карточке смены.</p>
            </div>
        </aside>
    </div>
@endsection
