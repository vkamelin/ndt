...
@section('content')
    <div class="space-y-6">
        <div class="panel p-6">
            ...
            <h1 class="mt-2 text-3xl font-semibold text-slate-900">Редактирование документа</h1>
            <p class="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
                Форма редактирования вынесена отдельно от карточки документа.
            </p>
        </div>
        ...
        <div class="panel p-6">
            <form method="post" action="{{ route('admin.documents.update', $document) }}" class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                ...
                <div class="space-y-2">
                    ...
                    <select id="document_type_id" name="document_type_id" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm">
                        @foreach ($documentTypes as $documentType)
                            <option value="{{ $documentType->id }}" @selected(old('document_type_id', $document->document_type_id) == $documentType->id)>{{ $documentType->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="space-y-2">
                ...
                <div class="space-y-2">
                    ...
                    <select id="employee_id" name="employee_id" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm">
                        ...
                        @foreach ($employees as $employee)
                            <option value="{{ $employee->id }}" @selected(old('employee_id', $document->employee_id) == $employee->id)>{{ $employee->fullName() }} — {{ $employee->object?->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="space-y-2">
                    ...
                    <select id="equipment_id" name="equipment_id" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm">
                        ...
                        @foreach ($equipment as $item)
                            <option value="{{ $item->id }}" @selected(old('equipment_id', $document->equipment_id) == $item->id)>{{ $item->name }} — {{ $item->object?->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="space-y-2">
                    ...
                    <select id="ndt_request_id" name="ndt_request_id" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm">
                        ...
                        @foreach ($requests as $requestItem)
                            <option value="{{ $requestItem->id }}" @selected(old('ndt_request_id', $document->ndt_request_id) == $requestItem->id)>{{ $requestItem->request_number }} — {{ $requestItem->object?->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="space-y-2">
                ...
                <div class="md:col-span-2 xl:col-span-3 space-y-2">
                    ...
                    <textarea id="comment" name="comment" rows="3" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm">{{ old('comment', $document->comment) }}</textarea>
                </div>
                <div class="md:col-span-2 xl:col-span-3 flex flex-wrap gap-3">
                    <button type="submit" class="rounded-full bg-brand-600 px-4 py-2 text-sm font-semibold text-white">Сохранить документ</button>
                    <a href="{{ route('admin.documents.show', $document) }}" class="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">Отмена</a>
                </div>
            </form>
        </div>
    </div>
