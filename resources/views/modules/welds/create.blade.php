...
@section('content')
    <div class="space-y-6">
        <div class="panel p-6">
            ...
            <h1 class="mt-2 text-3xl font-semibold text-slate-900">Создание стыка</h1>
            <p class="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
                Форма создания вынесена со списка, чтобы не перегружать экран.
            </p>
        </div>
        ...
        <div class="panel p-6">
            <form method="post" action="{{ route('admin.welds.store') }}" class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                ...
                <div class="md:col-span-2 xl:col-span-3 flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('admin.welds.index') }}" class="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">Отмена</a>
                </div>
            </form>
        </div>
    </div>
