...
@section('content')
    <div class="space-y-6">
        <div class="panel p-6">
            ...
            <h1 class="mt-2 text-3xl font-semibold text-slate-900">Редактирование карты РК</h1>
            <p class="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
                Форма редактирования перенесена отдельно. Сохранение выполняется через ту же карточку РК.
            </p>
        </div>
        ...
        <div class="panel p-6">
            <form method="post" action="{{ route('admin.radiography.store') }}" class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                ...
                <div class="space-y-2">
                    ...
                    <select id="ndt_result_id" name="ndt_result_id" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm">
                        @foreach ($ndtResults as $ndtResult)
                            <option value="{{ $ndtResult->id }}" @selected(old('ndt_result_id', $result->ndt_result_id) == $ndtResult->id)>{{ $ndtResult->weld?->weld_number }} — {{ $ndtResult->weld?->object?->name }} — {{ $ndtResult->method?->code?->label() }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="space-y-2">
                    ...
                    <select id="film_type_id" name="film_type_id" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm">
                        ...
                        @foreach ($filmTypes as $filmType)
                            <option value="{{ $filmType->id }}" @selected(old('film_type_id', $result->film_type_id) == $filmType->id)>{{ $filmType->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="space-y-2">
                    <label class="text-sm font-medium text-slate-700" for="barcode">Баркод</label>
                    <input id="barcode" name="barcode" value="{{ old('barcode', $result->barcode) }}" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm">
                </div>
                <div class="space-y-2">
                    <label class="text-sm font-medium text-slate-700" for="conclusion_number">Номер заключения</label>
                    <input id="conclusion_number" name="conclusion_number" value="{{ old('conclusion_number', $result->conclusion_number) }}" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm">
                </div>
                <div class="space-y-2">
                ...
                <div class="space-y-2">
                    ...
                    <input id="conclusion_date" type="date" name="conclusion_date" value="{{ old('conclusion_date', optional($result->conclusion_date)->format('Y-m-d')) }}" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm">
                </div>
                <div class="space-y-2">
                    <label class="text-sm font-medium text-slate-700" for="archive_location">Архивное место</label>
                    <input id="archive_location" name="archive_location" value="{{ old('archive_location', $result->archive_location) }}" class="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm">
                </div>
                ...
                <div class="md:col-span-2 xl:col-span-3 flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('admin.radiography.show', $result) }}" class="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">Отмена</a>
                </div>
            </form>
        </div>
    </div>
